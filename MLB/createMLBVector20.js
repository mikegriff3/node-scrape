        ...
        axios
          ...
          .then(
            axios.spread(async (homeSchedule, awaySchedule) => {
              ...
              // Training
              // var homeStarter = await getHomeStarterStats(homeGames);
              // var awayStarter = await getAwayStarterStats(awayGames);
              ...
              // console.log("HOME_SP_GP: ", homeStarter.gamesStarted);
              // console.log("AWAY_SP_GP: ", awayStarter.gamesStarted);
              if (
                homeStarter.gamesStarted >= 2 &&
                awayStarter.gamesStarted >= 2
              ) {
...
function getPastGamesStats(games, team) {
  ...
  var newArr = arr.slice(0, 20);
  if (arr.length > 0) {
    // Iterate last twenty games stats
...
function getHomeStarterStats(games, starter) {
  ...
  if (arr.length > 0) {
    ...
    if (starterGames.length > 0) {
      var newArr = starterGames.slice(0, 4);
      //console.log(newArr);
      if (newArr.length > 0) {
      ...
      starterStats.gamesStarted = newArr.length;
